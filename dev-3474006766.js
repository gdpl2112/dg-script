    ...
    if (context.getSender().getId() == 3474006766) {
        ...
        switch (okv[0]) {
            ...
            case "/req-get":
                ...
                break
            case "/test":
                sendToText("广度优先遍历（Breadth-First Search，简称BFS）是一种图的遍历算法。它从图的起始节点开始，逐层地访问与该节点直接相邻的节点，然后再逐层访问这些节点的邻接节点，以此类推，直到遍历完所有可达节点。\n" +
                    "\n" +
                    "BFS的具体步骤如下：\n" +
                    "\n" +
                    "创建一个空队列和一个空集合。\n" +
                    ...
                    "当队列不为空时，执行以下操作：\n" +
                    "从队列中取出一个节点，并访问该节点。\n" +
                    "将与该节点相邻且未被访问过的节点放入队列中，并将它们标记为已访问。\n" +
                    "将该节点加入到已访问节点的集合中。\n" +
                    "重复步骤3，直到队列为空。\n" +
                    "BFS通过使用队列来实现按层遍历的效果，保证了在遍历时按照节点的距离起始节点的层次进行访问。这样可以方便地求解最短路径、寻找连通分量、检测环路等问题。\n" +
                    "\n" +
                    "需要注意的是，BFS需要使用额外的数据结构来记录节点的访问状态，以避免重复访问同一节点或进入死循环。同时，BFS还可以应用在树的层次遍历中。\n" +
                    "\n" +
                    "总之，广度优先遍历通过队列实现按层遍历，逐层访问与当前节点相邻的未访问节点，用于解决一些与节点层次相关的问题。")
                break
            case "/repeat":
                if (okv.length !== 2) context.send("args size less 2")
                context.send(okv[1])
